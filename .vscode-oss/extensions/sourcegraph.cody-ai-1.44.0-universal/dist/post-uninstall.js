  ...
  return (observable) => {
    ...
    return new observable_default((observer) => {
      ...
      if (hasValue) {
        ...
        logDebug2("new subscriber, no buffered value to emit");
      }
      if (!subscription) {
...
var SourcegraphGraphQLAPIClient = class _SourcegraphGraphQLAPIClient {
  ...
  async queryPrompts({
    ...
    const response = await this.fetchSourcegraphAPI(
      hasIncludeViewerDraftsArg ? PROMPTS_QUERY : LEGACY_PROMPTS_QUERY_5_8,
      {
        ...
        first: first ?? 100,
        recommendedOnly,
        orderByMultiple: ["PROMPT_RECOMMENDED" /* PROMPT_RECOMMENDED */, "PROMPT_UPDATED_AT" /* PROMPT_UPDATED_AT */]
      },
      signal
    );
  ...
  fetchSourcegraphDotcomAPI(query, variables) {
    ...
    return wrapInActiveSpan(
      ...
      () => fetch2(url, {
        ...
        body: JSON.stringify({ query, variables }),
        headers
      }).then(verifyResponseCode).then((response) => response.json()).catch((error) => new Error(`error fetching Sourcegraph GraphQL API: ${error} (${url})`))
    );
  }
  ...
  fetchSourcegraphTestingAPI(body) {
    ...
    }).then(verifyResponseCode).then((response) => response.json()).catch((error) => new Error(`error fetching Testing Sourcegraph API: ${error} (${url})`));
  }
...
function handleExportModeTransforms(exportMode, events) {
...
var TelemetryRecorderProvider = class extends import_telemetry.TelemetryRecorderProvider {
  constructor(config, legacyBackcompatLogEventMode) {
    ...
    super(
      ...
      process.env.CODY_TELEMETRY_EXPORTER === "testing" ? TESTING_TELEMETRY_EXPORTER.withAnonymousUserID(config.clientState.anonymousUserID) : new GraphQLTelemetryExporter(legacyBackcompatLogEventMode),
      [
...
async function main() {
  ...
  if (uninstaller) {
    ...
    if (config && authStatus2) {
      ...
      const provider = process.env.CODY_TESTING ? new MockServerTelemetryRecorderProvider(config) : new TelemetryRecorderProvider(config, "connected-instance-only");
